...
while(my $file = readdir(DIR)){
  push @tables, $file if -f "$table_dir/$file";
}
closedir(DIR);

open(IN, "<$obj_table") or die "Cannot open table $obj_table. \n";
...
for(my $ind = 0; $ind < @tables; $ind ++){
  ...
  open(IN, "<$current_tab") or die "Cannot open table $current_tab. \n";
  <IN>; # skim title
  while(<IN>){
    chomp;
    s/[\r|\n]//g;
    ...
    if(!exists $tList{$_}) {
      $tList{$_} = $_;
      push(@{$com_list{$tmpArray[0]}}, [(@tmpArray)[1..$#tmpArray]]);
    }
